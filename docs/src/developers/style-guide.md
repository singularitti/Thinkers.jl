...
!!! info
    The style guide is always a work in progress, and not all Thinkers code
    follows the rules. When modifying Thinkers, please fix the style violations
...
Thinkers uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/Thinkers.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the Thinkers directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to Thinkers have
